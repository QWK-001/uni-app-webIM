	...
	<view class="search_input" v-if="search_chats">
		<view>
			...
			<input placeholder="搜索" placeholder-style="color:#9B9B9B;line-height:21px;font-size:15px;" auto-focus confirm-type="search" type="text" @confirm="onSearch" @input="onInput" :value="input_code"></input>
				<icon type="clear" size="12" @tap.native.stop="clearInput" v-if="show_clear"></icon>

		</view>
		<text @tap="cancel">取消</text>
	</view>

<view v-for="(item, index) in arr" :key="index" class="chat_list" :data-item="item" @tap.native.stop="del_chat">
	<swipe-delete>
		<view class="tap_mask" @tap.native.stop="into_chatRoom" :data-item="item">
			<view class="list_box">
