...
<view>
	...
	<scroll-view :class="'content ' + (gotop? (isIPX? 'goTopX': 'goTop'): 'goback')" enable-back-to-top :scroll-into-view="toView" scroll-y="true" scroll-with-animation="true" @scroll="onPageScroll" :style="'padding-bottom: ' + (isIPX?'270rpx':'226rpx')"> 
	    ...
	    <view v-for="(group, index) in listMain" :key="index" :data-id="group.id" :idAttr="( 'inToView'+group.id)"> 
	      <view class="address_top">{{group.region}}</view> 
	        <view v-for="(item, index2) in group.brands" :key="index2" :data-username="item.name" @tap.native.stop="delete_friend"> 
				<swipe-delete>
					<view class="tap_mask" @tap.native.stop="into_room" :data-username="item.name">
			          	<view class="address_bottom" :data-username="item.name" @tap.native.stop="into_room">
							<image src="../../static/images/theme@2x.png" @tap.native.stop="into_room" :data-username="item.name"></image>
				          	<text @tap.native.stop="into_room" :data-username="item.name">{{item.name}}</text>
			      		</view>
		      		</view>
	      		</swipe-delete>
	        </view> 
	  ...
	  <view class="orientation_region"> 
	   <!--  <view class="orientation">#</view> --> 
	     	 <block v-for="(item, index) in listMain" :key="index"> 
		        <view :class="'orientation_city ' + (isActive==item.id ? 'active':'' )" @tap="scrollToViewFn" :data-id="item.id">{{item.region}}
		        </view> 
	    	</block> 
	  </view>  
	</view>
